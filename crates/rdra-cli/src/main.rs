...
use rdra_core::{build_merged_model, resolve};
use rdra_emit::{
    csv::{ActorListCsvEmitter, EntityListCsvEmitter, RelationMatrixCsvEmitter},
    mermaid::{ErMermaidEmitter, RdraMermaidEmitter, StateMermaidEmitter},
    plantuml::{
...
#[derive(Parser)]
#[command(name = "rdra", about = "RDRA DSL compiler")]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
fn load_model(
    inputs: &[PathBuf],
) -> Result<(rdra_core::SemanticModel, Vec<rdra_core::Diagnostic>)> {
    ...
    match cli.command {
        ...
        Commands::Diagram {
            ...
            if matches!(kind, DiagramKind::Sequence) {
                let txs = rdra_core::infer_usecase_transactions(&model);
                for diag in rdra_core::tx_diagnostics(&model, &txs) {
                    eprintln!("warning: {}", diag.error);
                }
            }
            ...
            let diagram_text = match format {
                OutputFormat::Mermaid => match kind {
                    ...
                    DiagramKind::Sequence => {
                        eprintln!("warning: sequence diagram is not yet supported in Mermaid format; falling back to PlantUML");
                        SequenceDiagramEmitter.emit(&model, &view)?
                    }
                },
                _ => match kind {
            ...
            match format {
                ...
                OutputFormat::Svg => {
                    use rdra_render::{render_to_file, PlantumlCliRenderer, RenderFormat};
                    let renderer =
...
fn format_entities(model: &rdra_core::SemanticModel, format: &ListFormat) -> Result<String> {
    use rdra_core::model::ColumnType;
