...
use rdra_core::model::{
    ActorKey, BucKey, ColumnType, EntityKey, NodeRef, RelKind, ScreenKey, SemanticModel, UseCaseKey,
};
use rdra_core::tx::infer_usecase_transactions;
...
impl Emitter for SequenceDiagramEmitter {
    fn emit(&self, model: &SemanticModel, view: &View) -> Result<String, EmitError> {
            ...
            match &rel.kind {
                ...
                RelKind::Performs => {
                    match (&rel.from, &rel.to) {
                        ...
                        // performs(actor, usecase) — 直接参照の場合
                        (NodeRef::Actor(ak), NodeRef::UseCase(uk)) => {
                            // BUCなしの直接アクターとして uc_to_bucs の代替に記録
                            // ここでは dummy BucKey を使わず専用マップを設ける
                            let _ = (ak, uk); // 後続の direct_actor_of で処理
                        }
                        _ => {}
                    }
                }
        ...
        // performs(actor, usecase) 直接参照マップ
        ...
        let uc_tx_map: HashMap<UseCaseKey, &rdra_core::UsecaseTx> =
            uc_txs.iter().map(|t| (t.usecase, t)).collect();

        // ── 表示対象ユースケース（書き込みありのもの、可視なもの） ─────────
        let mut uc_list: Vec<(UseCaseKey, &rdra_core::model::UseCase)> = model
            .use_cases
            .iter()
            .filter(|(k, _)| {
        ...
        // 参加者宣言（アクター）
        let mut actors_sorted: Vec<(ActorKey, &rdra_core::model::Actor)> = model
            .actors
            .iter()
        ...
        // 参加者宣言（エンティティ）
        let mut ents_sorted: Vec<(EntityKey, &rdra_core::model::Entity)> = model
            .entities
            .iter()
        ...
        // 参加者宣言（画面）
        let mut scrs_sorted: Vec<(ScreenKey, &rdra_core::model::Screen)> = model
            .screens
            .iter()
